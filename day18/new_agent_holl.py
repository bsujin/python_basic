...
# FLAG_MODEL = Trus
FLAG_MODEL = False
# 데이터를 모으는것 (데이터가 100개 이상 모아지면 fit)
TRAIN_START = 1000
# 일정량 도달되면 삭제 (큐의 용량 )  - 메모리양 
MEMORY_DEQUE = 2000
...
# 1판의 게임 값 - 내가 둔다 -> 한판 두고 난 다음에 올려준다 
	def step(self, mine):
...
class Agent:
	def __init__(self):
		...
		if self.load_model:
			...
			# 타겟 모델 - 기존의 모델보다 더 좋은 결과일 때 넣어준다 
...
# 			now_state = next_state
		...
		if flag_end:
			...
			pylab.plot(arr_episode, arr_score, 'b')	# 그래프를 그려준다 
